...
import 'package:music_player/bloc/home_bloc/home_event.dart';
import 'package:music_player/bloc/home_bloc/home_state.dart';
import 'package:music_player/db_helper/db_helper.dart';
import 'package:music_player/model/audio_file_model.dart';
import 'package:music_player/view_model/services/audio_query_services.dart';
...
class HomeBloc extends Bloc<HomeEvents,HomeState>{
 final List<AudioFile> songList=[];
  ...
  Future<void> _onSongFileEvent(GetSongEvent event,Emitter<HomeState> emit) async {
    ...
    try{
      songList.clear();
      for(var item in (await AudioFileQueries.getFiles('/storage/emulated/0/music_player'))){
        songList.add(item);
      }
    }catch(_){
  ...
  Future<void> _onAddToAlbumEvent(AddToAlbum event,Emitter<HomeState> emit) async {
    try{
      if(Directory('/storage/emulated/0/music_player').existsSync()){
      }else{
        Directory('/storage/emulated/0/music_player').createSync(recursive: true);
      }

      final bool alreadyExist=File('/storage/emulated/0/music_player/${event.file.name!}').existsSync();
      if(alreadyExist){
      }else{
        File(event.file.path!).copySync('/storage/emulated/0/music_player/${event.file.name!}');
